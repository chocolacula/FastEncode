...
private:
	...
	template <int N>
	constexpr static inline uintptr_t RoundUp(uintptr_t pointer)
	{
	...
	// N must be a power of two.
	template <int N>
	constexpr static inline const uint8_t * RoundUp(const uint8_t * pointer)
	{
		static_assert((N & (N - 1)) == 0, "N must be a power of two");
	...
	static constexpr const ptrdiff_t kPrefetchHorizon = 256;

	static inline void RequestPrefetch(const uint8_t* address)
	{
...
public:
	static inline uint32_t Calculate(const uint8_t* bytes, uint32_t size, uint32_t initial = 0xFFFFFFFF)
	{
		const uint8_t* p = bytes;
		const uint8_t* e = bytes + size;
		uint32_t l = initial ^ kCRC32Xor;
