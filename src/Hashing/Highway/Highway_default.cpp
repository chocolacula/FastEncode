#include "Highway_default.h"

#include <stdlib.h>

void Highway_default::HighwayHashReset(const uint64_t key[4], HighwayHashState* state)
{
...
void Highway_default::ZipperMergeAndAdd(const uint64_t v1, const uint64_t v0,	uint64_t* add1, uint64_t* add0)
{
...
void Highway_default::Update(const uint64_t lanes[4], HighwayHashState* state)
{
	for (auto i = 0; i < 4; ++i)
	{
...
uint64_t Highway_default::Read64(const uint8_t* src)
{
...
void Highway_default::HighwayHashUpdatePacket(const uint8_t* packet, HighwayHashState* state)
{
	uint64_t lanes[4];
...
void Highway_default::Rotate32By(uint64_t count, uint64_t lanes[4])
{
	int i;
	for (i = 0; i < 4; ++i) {
...
void Highway_default::HighwayHashUpdateRemainder(const uint8_t* bytes, const size_t size_mod32, HighwayHashState* state)
{
	int i;
...
void Highway_default::Permute(const uint64_t v[4], uint64_t* permuted)
{
...
void Highway_default::PermuteAndUpdate(HighwayHashState* state)
{
	uint64_t permuted[4];
...
void Highway_default::ModularReduction(uint64_t a3_unmasked, uint64_t a2, uint64_t a1, uint64_t a0, uint64_t* m1, uint64_t* m0)
{
...
uint32_t Highway_default::HighwayHashFinalize32(HighwayHashState* state)
{
	for (auto i = 0; i < 4; i++)
	{
...
uint64_t Highway_default::HighwayHashFinalize64(HighwayHashState* state)
{
	for (auto i = 0; i < 4; i++)
	{
		PermuteAndUpdate(state);
...
void Highway_default::HighwayHashFinalize128(HighwayHashState* state, uint64_t hash[2])
{
...
void Highway_default::HighwayHashFinalize256(HighwayHashState* state, uint64_t hash[4])
{
...
void Highway_default::ProcessAll(const uint8_t* data, size_t size, const uint64_t key[4], HighwayHashState* state)
{	
...
uint32_t Highway_default::HighwayHash32(const uint8_t* data, size_t size, const uint64_t key[4])
{
	HighwayHashState state;
	ProcessAll(data, size, key, &state);
	return HighwayHashFinalize32(&state);
}

uint64_t Highway_default::HighwayHash64(const uint8_t* data, size_t size, const uint64_t key[4])
{
	HighwayHashState state;
	ProcessAll(data, size, key, &state);
...
void Highway_default::HighwayHash128(const uint8_t* data, size_t size, const uint64_t key[4], uint64_t hash[2])
{
	HighwayHashState state;
	ProcessAll(data, size, key, &state);
...
void Highway_default::HighwayHash256(const uint8_t* data, size_t size, const uint64_t key[4], uint64_t hash[4])
{
	HighwayHashState state;
	ProcessAll(data, size, key, &state);
...
void Highway_default::HighwayHashCatStart(const uint64_t key[4], HighwayHashCat* state)
{
	HighwayHashReset(key, &state->state);
	state->num = 0;
}

void Highway_default::HighwayHashCatAppend(const uint8_t* bytes, size_t num, HighwayHashCat* state)
{
	size_t i;
	if (state->num != 0)
	{
		size_t num_add = num > (32u - (size_t)state->num) ? (32u - (size_t)state->num) : num;
...
uint32_t Highway_default::HighwayHashCatFinish32(const HighwayHashCat* state)
{
...
uint64_t Highway_default::HighwayHashCatFinish64(const HighwayHashCat* state)
{
	HighwayHashState copy = state->state;
...
void Highway_default::HighwayHashCatFinish128(const HighwayHashCat* state, uint64_t hash[2])
{
	HighwayHashState copy = state->state;
...
void Highway_default::HighwayHashCatFinish256(const HighwayHashCat* state, uint64_t hash[4])
{
	HighwayHashState copy = state->state;
